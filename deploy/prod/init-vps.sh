...
echo -e "${YELLOW}======================================================================${NC}"
echo -e "Next steps to deploy the application on the shared VPS edge:"
echo -e "1. From the repository root, create your production environment file:"
echo -e "   ${GREEN}cp deploy/prod/.env.vps-edge.example deploy/prod/.env${NC}"
echo -e "2. Edit ${GREEN}deploy/prod/.env${NC} and fill in a stable ${GREEN}SECRET_ENCRYPTION_KEY${NC} and a strong ${GREEN}REDIS_PASSWORD${NC}."
echo -e "3. Launch Redis and the API behind the host-level Caddy edge:"
echo -e "   ${GREEN}docker compose -f deploy/prod/docker-compose.yml -f deploy/prod/docker-compose.vps-edge.yml up -d --build redis api${NC}"
echo -e "4. Monitor boot status:"
echo -e "   ${GREEN}docker compose -f deploy/prod/docker-compose.yml -f deploy/prod/docker-compose.vps-edge.yml logs -f api redis${NC}"
echo -e "5. Verify local and public health endpoints:"
echo -e "   ${GREEN}curl http://127.0.0.1:18080/healthz${NC}"
echo -e "   ${GREEN}curl https://api.secret.quorix.io.vn/healthz${NC}"
echo -e "6. Ensure the host-level Caddy edge proxies to ${GREEN}127.0.0.1:18080${NC} and overwrites forwarded headers at the boundary."
echo -e "${YELLOW}======================================================================${NC}"
